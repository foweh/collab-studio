...
// Create the test script as a file
const testCode = `
  ...
  if (!stateReceived) {
    ...
    setTimeout(() => {
      ...
      s2.on('fenjing:scenes-update', (scenes) => {
        console.log('S2 GOT scenes-update: ' + scenes.length + ' scenes');
        const last = scenes[scenes.length - 1];
        console.log('S2 Last scene name: ' + last.name);
        console.log('SYNC ' + (last.name === 'TEST_SYNC' ? 'OK' : 'FAILED'));
      });
...
conn.on('ready', () => {
  const cmd = 'echo ' + JSON.stringify(b64) + ' | base64 -d > /tmp/test_fenjing.js && cd /root/collab-studio/collab-studio && node /tmp/test_fenjing.js';
